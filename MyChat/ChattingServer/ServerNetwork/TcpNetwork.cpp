...
namespace ServerNetwork
{
	...
	ServerNetwork::NETWORK_ERROR_CODE TcpNetwork::SendData(const int sessionIndex, const short packetId, const short size, const char* pMsg)
	{
		...
		if ((pos + size + PACKET_HEADER_SIZE) > m_Config.maxClientSendBufferSize) {
			return NETWORK_ERROR_CODE::CLIENT_SEND_BUFFER_FULL;
		}
	...
	ServerNetwork::RecvPacketInfo TcpNetwork::GetPacketInfo()
	{
	...
	ServerNetwork::NETWORK_ERROR_CODE TcpNetwork::InitServerSocket()
	{
	...
	ServerNetwork::NETWORK_ERROR_CODE TcpNetwork::BindListen(short port, int backlogCount)
	{
		...
		m_pRefLogger->Write(LOG_TYPE::LOG_INFO, "%s | Listen. ServerSockfd(%d)", __FUNCTION__, m_ServerSockfd);
		return NETWORK_ERROR_CODE::NONE;
	}
	...
	ServerNetwork::NETWORK_ERROR_CODE TcpNetwork::NewSession()
	{
		...
		if (client_sockfd < 0)
		{
			m_pRefLogger->Write(LOG_TYPE::LOG_ERROR, "%s | Wrong socket %d cannot accept", __FUNCTION__, client_sockfd);
		...
		{
			m_pRefLogger->Write(LOG_TYPE::LOG_WARN, "%s | client_sockfd(%d)  >= MAX_SESSION", __FUNCTION__, client_sockfd);
	...
	void TcpNetwork::ConnectedSession(const int sessionIndex, const int fd, const char* pIP)
	{
		...
		m_pRefLogger->Write(LOG_TYPE::LOG_INFO, "%s | New Session. FD(%d), m_ConnectSeq(%d), IP(%s)", __FUNCTION__, fd, m_ConnectSeq, pIP);
	}
	...
	ServerNetwork::NETWORK_ERROR_CODE TcpNetwork::RecvSocket(const int sessionIndex)
	{
		auto& session = m_ClientSessionPool[sessionIndex];
		...
		{
			return NETWORK_ERROR_CODE::RECV_PROCESS_NOT_CONNECTED;
		}
		...
		auto recvSize = recv(fd, &session.pRecvBuffer[recvPos], (MAX_PACKET_SIZE * 2), 0); // *2는 그냥 적당히 버퍼 사이즈를 2배로..

		if (recvSize == 0) // 패킷이 짤린것임
		{
			return NETWORK_ERROR_CODE::RECV_REMOTE_CLOSE;
		}
		...
		if (recvSize < 0)
		{
			...
			{
				return NETWORK_ERROR_CODE::RECV_API_ERROR;
			}
			else
			{
				return NETWORK_ERROR_CODE::NONE;
	...
	ServerNetwork::NetworkError TcpNetwork::FlushSendBuff(const int sessionIndex)
	{
		auto& session = m_ClientSessionPool[sessionIndex];
		auto fd = static_cast<SOCKET>(session.SocketFD);
		...
		if (session.IsConnected() == false)
		{
			return NetworkError(NETWORK_ERROR_CODE::CLIENT_FLUSH_SEND_BUFF_REMOTE_CLOSE);
		}

		auto result = SendSocket(fd, session.pSendBuffer, session.SendSize);

		if (result.Error != NETWORK_ERROR_CODE::NONE) {
			return result;
		}
	...
	ServerNetwork::NetworkError TcpNetwork::SendSocket(const SOCKET fd, const char* pMsg, const int size)
	{
		NetworkError result(NETWORK_ERROR_CODE::NONE);
		auto rfds = m_Readfds;
		...
		if (result.Vlaue <= 0)
		{
			result.Error = NETWORK_ERROR_CODE::SEND_SIZE_ZERO;
		}
